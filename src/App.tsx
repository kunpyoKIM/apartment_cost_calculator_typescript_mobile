...
export default function App() {
  ...
  const stepComplete: Record<StepKey, boolean> = {
    area: num(apartmentArea) + num(officetelArea) + num(retailArea) + num(basementArea) > 0,
  ...
  const reset = () => {
    ...
    setRegion(''); setHousingType(''); setParkingType(''); setGreenType(''); setPfType(''); setLandCost(''); setApartmentArea(''); setOfficetelArea(''); setRetailArea(''); setBasementArea('');
  ...
  const moveToStep = (step: StepKey) => {
    ...
    if (step === 'unit' || step === 'region' || step === 'housing') setTab('unit');
    if (step === 'parking' || step === 'green') setTab('area');
  ...
  return <div className={dark ? 'appShell dark' : 'appShell'}><main className="phoneFrame">
    ...
    <section className="summaryCard"><span>총 사업비 추정액</span><strong>{fmt(calc.total)} 천원</strong><div><em>평당 {fmt(calc.pyeongUnit)} 천원</em><em>대출 {fmt(calc.loanAmount)} 천원</em></div></section>
    <section className="contentArea">
      {tab === 'home' && <><section className="mobileCard"><div className="cardTitle">진행순서 바로가기</div><div className="stepButtons">{steps.map(s => <button key={s.key} type="button" className="stepButton" onClick={() => moveToStep(s.key)}><i className={stepComplete[s.key] ? 'lamp on' : 'lamp'} /><span>{s.title}</span><ChevronRight size={18}/></button>)}</div></section><section className="mobileCard"><div className="cardTitle">빠른 결과</div><ResultLine label="아파트 직접공사비" value={`${fmt(calc.apartment)} 천원`} /><ResultLine label="오피스텔 공사비" value={`${fmt(calc.officetel)} 천원`} /><ResultLine label="근린상가 공사비" value={`${fmt(calc.retail)} 천원`} /><ResultLine label="PF 금융비" value={`${fmt(calc.financeCost)} 천원`} /></section><section className="mobileCard"><button className="resetButton" onClick={reset} type="button"><RotateCcw size={18}/> 전체 초기화</button></section></>}
      {tab === 'unit' && <>{(focusStep === 'unit' || focusStep === 'area') && <section className="mobileCard"><div className="cardTitle">국토부 단가 입력</div><Field label="조회연도" value={molitSearchYear} onChange={setMolitSearchYear}/><button className="actionButton" onClick={() => { setMolitYear(molitSearchYear); setUseMolitBase(true); }} type="button">검색</button><Field label="적용연도" value={molitYear} onChange={setMolitYear}/><Field label="기본형건축비" value={molitBaseCost} onChange={setMolitBaseCost} comma suffix="천원/평"/><Field label="지하층건축비" value={molitBasementCost} onChange={setMolitBasementCost} comma suffix="천원/평"/><button className="actionButton green" type="button">저장/적용</button><div className="toggleRow"><button className={useMolitBase ? 'pill active' : 'pill'} onClick={() => setUseMolitBase(true)}>국토부 단가</button><button className={!useMolitBase ? 'pill active' : 'pill'} onClick={() => setUseMolitBase(false)}>실무단가</button></div></section>}{(focusStep === 'region' || focusStep === 'area') && <OptionList title="지역 선택" items={regionRates} value={region} onChange={setRegion}/>} {(focusStep === 'housing' || focusStep === 'area') && <OptionList title="주거유형 선택" items={housingTypes} value={housingType} onChange={setHousingType}/>}</>}
      {tab === 'area' && <>{focusStep === 'area' && <section className="mobileCard"><div className="cardTitle">면적 및 단가 입력</div><Field label="토지대" value={landCost} onChange={setLandCost} comma suffix="천원"/><Field label="아파트 평면적" value={apartmentArea} onChange={setApartmentArea} comma suffix="평"/><Field label="오피스텔 평면적" value={officetelArea} onChange={setOfficetelArea} comma suffix="평"/><Field label="근린상가 평면적" value={retailArea} onChange={setRetailArea} comma suffix="평"/><Field label="지하층 평면적" value={basementArea} onChange={setBasementArea} comma suffix="평"/><Field label="물가 상승률" value={increaseRate} onChange={setIncreaseRate} suffix="%"/></section>}{(focusStep === 'parking' || focusStep === 'area') && <OptionList title="지하주차장 유형 선택" items={parkingTypes} value={parkingType} onChange={setParkingType}/>} {(focusStep === 'green' || focusStep === 'area') && <OptionList title="친환경 제로에너지" items={greenOptions} value={greenType} onChange={setGreenType} unit="%"/>}</>}
      {tab === 'pf' && <><section className="mobileCard"><div className="cardTitle">PF 수지 입력</div><Field label="대출비율" value={loanRatio} onChange={setLoanRatio} suffix="%"/><Field label="금리" value={interestRate} onChange={setInterestRate} suffix="%"/><Field label="사업기간" value={periodMonths} onChange={setPeriodMonths} suffix="개월"/><Field label="예상매출액" value={salesRevenue} onChange={setSalesRevenue} comma suffix="천원"/></section><OptionList title="금융기관 검토 기준 선택" items={pfOptions} value={pfType} onChange={setPfType}/></>}
      {tab === 'result' && <><section className="mobileCard"><div className="resultHeader"><div className="cardTitle">세부 산출내역</div><button className="printButton noPrint" onClick={() => window.print()} type="button"><Printer size={17}/> PDF 프린트</button></div><ResultLine label="최종 지상 적용단가" value={`${fmt(calc.baseRate)} 천원/평`}/><ResultLine label="최종 지하 적용단가" value={`${fmt(calc.basementRate)} 천원/평`}/><ResultLine label="아파트 직접공사비" value={`${fmt(calc.apartment)} 천원`}/><ResultLine label="아파트 경비 50%" value={`${fmt(calc.apartmentExpense)} 천원`}/><ResultLine label="오피스텔 공사비" value={`${fmt(calc.officetel)} 천원`}/><ResultLine label="근린상가 공사비" value={`${fmt(calc.retail)} 천원`}/><ResultLine label="지하층건축비" value={`${fmt(calc.basement)} 천원`}/><ResultLine label="친환경 제로에너지" value={`${fmt(calc.greenCost)} 천원`}/><ResultLine label="물가 상승 반영액" value={`${fmt(calc.increaseCost)} 천원`}/><ResultLine label="PF 금융비" value={`${fmt(calc.financeCost)} 천원`}/></section><section className="mobileCard"><div className="cardTitle">사업성 요약</div><ResultLine label="총 사업비" value={`${fmt(calc.total)} 천원`}/><ResultLine label="평균 평당 단가" value={`${fmt(calc.pyeongUnit)} 천원/평`}/><ResultLine label="예상 손익" value={`${fmt(calc.profit)} 천원`}/><ResultLine label="손익률" value={`${calc.profitRate.toFixed(2)}%`}/></section></>}
    </section>
